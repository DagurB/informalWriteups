...
class Feistel:
	...
	@staticmethod
	def _f_generic(l: int, r: int, key: int, n: int = 64):
		return l ^ sbox(r ^ key, n=n)
	...
	def _encrypt_block_custom(self, pt: bytes, rounds: int, start: int = 0) -> bytes:
		assert len(pt) == self._block_size
		l, r = int.from_bytes(pt[:self._block_size // 2], "big"), int.from_bytes(pt[self._block_size // 2:], "big")
...
if __name__ == "__main__":
	...
	cipher = Feistel(key, rounds=6, block_size=16)
